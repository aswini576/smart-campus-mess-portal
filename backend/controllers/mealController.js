...
const selectMeals = `SELECT id AS _id, meal_type AS mealType, food_name AS foodName, food_category AS foodCategory,
  ...
  booking_close_time AS closingTime, is_available AS isAvailable,
...
function values(body) {
  ...
  return [body.mealType, body.foodName.trim(), body.foodCategory, dateTime(mealDate, '12:00:00'), dateTime(mealDate, closingTime), openingTime, closingTime, body.isAvailable !== false, Number(body.price || 0), Number(body.originalCost || 0), Number(body.messPaymentAmount || 0), JSON.stringify(body.ingredients || [])];
}
...
async function createMeal(request, response, next) {
  try {
    ...
    const [result] = await pool.execute('INSERT INTO meals (meal_type, food_name, food_category, meal_date, booking_deadline, booking_open_time, booking_close_time, is_available, price, original_cost, mess_payment_amount, ingredients) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', values(request.body));
...
async function createMealOptions(request, response, next) {
  ...
  try {
    ...
    const sql = 'INSERT INTO meals (meal_type, food_name, food_category, meal_date, booking_deadline, booking_open_time, booking_close_time, is_available, price, original_cost, mess_payment_amount, ingredients) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)';
...
async function updateMeal(request, response, next) {
  try {
    ...
    await pool.execute('UPDATE meals SET meal_type = ?, food_name = ?, food_category = ?, meal_date = ?, booking_deadline = ?, booking_open_time = ?, booking_close_time = ?, is_available = ?, price = ?, original_cost = ?, mess_payment_amount = ?, ingredients = ? WHERE id = ?', [...values(request.body), mealId]);
