...
CampusBite is a full-stack campus dining application for booking meals, managing menus and inventory, reducing food waste, and monitoring operations. It provides separate student, Mess Chief, and Admin experiences backed by an Express/MySQL API.
...
```
React + Vite frontend  →  Express REST API  →  MySQL
...
> Inventory usage and meal-recovery operations use MySQL transactions and InnoDB foreign keys.
...
Start MySQL and create a database user with permission to create the configured database. The backend creates the database and all required tables automatically when it starts.
...
- Common invalid IDs, duplicate records, and validation failures return safe 4xx responses.
...
Before deployment, test login for each role and exercise the required CRUD flows against the target MySQL deployment. Verify `/api/health` returns `{ "status": "ok", "database": "connected" }`.
