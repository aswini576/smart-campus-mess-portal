...
import { deleteMessPayment, getMessPayments, markMessPaymentPaid } from '../services/chiefService';
...
function MessPaymentsPage() {
  ...
  const rows = (payments || []).map((payment) => ({ id: payment._id, bulkDelete: () => deleteMessPayment(payment.weekStart), week: weekLabel(payment.weekStart), amount: money(payment.payableAmount), paymentDate: payment.paymentDate ? new Date(payment.paymentDate).toLocaleString() : '—', status: <Chip size="small" label={payment.status === 'paid' ? 'Paid' : 'Pending'} color={payment.status === 'paid' ? 'success' : 'warning'} />, action: payment.status === 'paid' ? '—' : <AppButton size="small" disabled={updating === payment.weekStart} onClick={() => markPaid(payment.weekStart)}>{updating === payment.weekStart ? 'Saving…' : 'Mark as Paid'}</AppButton> }));
